...
# ─── Live-positions cache (used by get_open_positions_live) ──────────────────
_live_positions_cache = {}
_live_positions_cache_time = {}
LIVE_POSITIONS_CACHE_TTL = 8   # seconds — prevents hammering when UI polls fast

# ─── Batch open-orders cache (replaces per-position API calls) ────────────────
_open_orders_cache = {}         # {user_id: [orders]}
_open_orders_cache_time = {}    # {user_id: timestamp}
OPEN_ORDERS_CACHE_TTL = 15      # seconds

# ─── Balance / wallet cache ───────────────────────────────────────────────────
_balance_cache = {}             # {user_id: (balance, margin_used)}
_balance_cache_time = {}        # {user_id: timestamp}
BALANCE_CACHE_TTL = 30          # seconds

# TTL cache + ban-aware short-circuit for conditional-orders endpoint.
# 15-second TTL prevents duplicate UI pollers from hammering Binance.
_conditional_cache = {}          # {user_id: (ts_ms, [orders])}
_conditional_ban_until = 0       # ms epoch; while now < this, skip the call
CONDITIONAL_CACHE_MS = 15_000   # 15 seconds — was 1000 (1 ms), the main culprit

# ─── Global Binance API ban / rate-limit state ────────────────────────────────
_api_ban_until = 0              # ms epoch; global ban covers ALL endpoints
_api_call_counter = 0           # calls in the current 60-second window
_api_call_window_start = 0.0    # epoch seconds when the window opened
API_CALLS_PER_MIN_LIMIT = 800   # Binance allows ~1 200/min; stay well below
...
def _is_api_available():
    """Return True if we are allowed to make a Binance API call right now."""
    global _api_ban_until, _api_call_counter, _api_call_window_start
    now_ms = int(time.time() * 1000)
    now_s  = time.time()

    # Hard ban set by Binance -1003
    if now_ms < _api_ban_until:
        remaining = (_api_ban_until - now_ms) / 1000
        print(f"🚫 Binance IP ban active — skipping call ({remaining:.0f}s remaining)")
        return False

    # Rolling 60-second self-throttle
    if now_s - _api_call_window_start > 60:
        _api_call_counter = 0
        _api_call_window_start = now_s

    if _api_call_counter >= API_CALLS_PER_MIN_LIMIT:
        print(f"⚠️ Self-throttle: {_api_call_counter} calls/60s — pausing")
        return False
...
def _handle_binance_ban(error_msg):
    """Parse Binance -1003 ban-until timestamp and store it globally.
    Returns True if a ban was detected."""
    global _api_ban_until
    now_ms = int(time.time() * 1000)
    msg = str(error_msg or "")
    try:
        if "banned until" in msg:
            ban_ts = int(msg.split("banned until")[1].split(".")[0].strip())
            _api_ban_until = max(_api_ban_until, ban_ts)
            print(f"🚫 Binance IP ban until {datetime.fromtimestamp(ban_ts / 1000)} stored globally")
            return True
    except Exception:
        pass
    if "-1003" in msg:
        _api_ban_until = max(_api_ban_until, now_ms + 60_000)
        print("🚫 Binance -1003 detected — backing off 60 s")
        return True
    return False
...
def get_all_exchange_symbols(user_id=None):
    ...
    # 10-minute cache for symbol list — it barely ever changes
    if _symbol_cache and (now - _symbol_cache_time) < 600:
...
def get_max_leverage(symbol, user_id=None):
    ...
    # 5-minute cache for leverage — it barely changes
    if cache_key in _leverage_cache and (now - _leverage_cache_time.get(cache_key, 0)) < 300:
        return _leverage_cache[cache_key]

    if not _is_api_available():
        return _leverage_cache.get(cache_key) or KNOWN_LEVERAGE_MAP.get(symbol, 20)
...
def _get_all_open_orders_batch(user_id=None):
    """
    Fetch ALL open orders for the account in ONE API call and cache them.
    Returns a dict: {symbol: [orders]} for fast per-symbol lookup.
    Previously the code called get_open_orders_for_symbol() once per position
    which was N Binance REST calls every 5 seconds — the main cause of -1003 bans.
    """
    global _open_orders_cache, _open_orders_cache_time
    now = time.time()
    cache_key = str(user_id or 'public')

    if cache_key in _open_orders_cache and (now - _open_orders_cache_time.get(cache_key, 0)) < OPEN_ORDERS_CACHE_TTL:
        return _open_orders_cache[cache_key]

    if not _is_api_available():
        return _open_orders_cache.get(cache_key, {})
    ...
    try:
        ...
        _open_orders_cache[cache_key] = by_symbol
        _open_orders_cache_time[cache_key] = now
        return by_symbol
    except Exception as e:
        _handle_binance_ban(str(e))
        print(f"Error fetching batch open orders: {e}")
        return _open_orders_cache.get(cache_key, {})
...
def get_open_positions(user_id=None, force_refresh=False):
    ...
    # ✅ FIX: Increased TTL from 5s → 15s to drastically reduce API calls.
    # Live PnL/mark-price still comes from the WebSocket on the frontend.
    if cache_key in _positions_cache and (current_time - _positions_cache_time.get(cache_key, 0)) < 15:
        return _positions_cache[cache_key]

    if not _is_api_available():
        return _positions_cache.get(cache_key, [])
    ...
    try:
        ...
        # ✅ FIX: Fetch ALL open orders in ONE batch call instead of once per position
        all_open_orders = _get_all_open_orders_batch(user_id)
        ...
        for pos in positions:
            ...
            if abs(position_amt) > 0:
                ...
                sym = pos.get('symbol')
                # ✅ FIX: Use pre-fetched batch instead of individual API call per symbol
                open_orders = all_open_orders.get(sym, [])
                ...
                open_positions.append({
                    ...
                    'dashboard_margin_ratio': dashboard_margin_ratio,
                    'entry_price': entry_price,
                    'mark_price': mark_price,
                    'unrealized_pnl': unrealized_pnl,
                    'roi_percent': roi_percent,
                    'dashboard_roi_percent': dashboard_roi_percent,
                    'leverage': leverage,
...
def get_open_positions_live(user_id=None):
    """
    Live position data with a short (8-second) cache.
    Previously had ZERO cache — every /api/liquidation_prices request
    hit Binance REST directly, which was the second biggest cause of -1003 bans.
    """
    global _live_positions_cache, _live_positions_cache_time
    cache_key = str(user_id or 'public')
    now = time.time()

    # ✅ FIX: 8-second cache prevents burst hammering from the UI poller
    if cache_key in _live_positions_cache and (now - _live_positions_cache_time.get(cache_key, 0)) < LIVE_POSITIONS_CACHE_TTL:
        return _live_positions_cache[cache_key]

    if not _is_api_available():
        return _live_positions_cache.get(cache_key, [])
    ...
    try:
        ...
        # Reuse the batch open-orders cache for live positions too
        all_open_orders = _get_all_open_orders_batch(user_id)
        ...
        _live_positions_cache[cache_key] = open_positions
        _live_positions_cache_time[cache_key] = now
        return open_positions
    except Exception as e:
        _handle_binance_ban(str(e))
...
def get_open_orders_for_symbol(symbol, user_id=None):
    """Returns open orders for a single symbol using the batch cache."""
    all_orders = _get_all_open_orders_batch(user_id)
    return all_orders.get(symbol, [])
...
def cancel_open_order(symbol, order_id, user_id=None):
    try:
        ...
        # Invalidate batch orders cache
        cache_key = str(user_id or 'public')
        _open_orders_cache.pop(cache_key, None)
        _open_orders_cache_time.pop(cache_key, None)
...
def get_all_open_conditional_orders(user_id=None):
    ...
    # Serve from 15-second TTL cache — kills duplicate-poll storms.
    ...
    if now_ms < _conditional_ban_until or now_ms < _api_ban_until:
...
def cancel_order(symbol, order_id, user_id=None):
    try:
        ...
        try:
            ...
            # Invalidate open-orders batch cache
            cache_key = str(user_id or 'public')
            _open_orders_cache.pop(cache_key, None)
            _open_orders_cache_time.pop(cache_key, None)
...
def execute_trade_action(balance, symbol, side, entry, order_type, sl_type, sl_value, sizing, user_units, user_lev, margin_mode, tp1, tp1_pct, tp2, user_id=None):
    ...
    try:
        ...
        leverage_attempts = list(dict.fromkeys(leverage_attempts))
        ...
        available_margin = float(balance) * 0.95
        ...
        try:
            sl_variants = [
                {"symbol": symbol, "side": x_side, "type": "STOP_MARKET", "stopPrice": sl_p, "closePosition": True, "workingType": "MARK_PRICE"},
                {"symbol": symbol, "side": x_side, "type": "STOP_MARKET", "stopPrice": sl_p, "closePosition": True},
                {"symbol": symbol, "side": x_side, "type": "STOP_MARKET", "stopPrice": sl_p, "quantity": qty, "reduceOnly": True, "workingType": "MARK_PRICE"},
                {"symbol": symbol, "side": x_side, "type": "STOP_MARKET", "stopPrice": sl_p, "quantity": qty, "reduceOnly": True},
                {"symbol": symbol, "side": x_side, "type": "STOP", "stopPrice": sl_p, "price": sl_p, "quantity": qty, "reduceOnly": True, "timeInForce": "GTC", "workingType": "MARK_PRICE"},
                {"symbol": symbol, "side": x_side, "type": "STOP", "stopPrice": sl_p, "price": sl_p, "quantity": qty, "reduceOnly": True, "timeInForce": "GTC"},
            ]
            ...
            if not sl_created:
                sl_algo_variants = [
                    {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "STOP_MARKET", "triggerPrice": sl_p, "closePosition": "true", "workingType": "MARK_PRICE"},
                    {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "STOP_MARKET", "triggerPrice": sl_p, "quantity": qty, "reduceOnly": "true", "workingType": "MARK_PRICE"},
                    {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "STOP", "triggerPrice": sl_p, "price": sl_p, "quantity": qty, "reduceOnly": "true", "timeInForce": "GTC", "workingType": "MARK_PRICE"},
                ]
        ...
        try:
            if tp1 > 0 and ((side=="LONG" and tp1>entry) or (side=="SHORT" and tp1<entry)):
                ...
                if tp1_qty > 0:
                    ...
                    tp1_variants = [
                        {"symbol": symbol, "side": x_side, "type": "TAKE_PROFIT_MARKET", "stopPrice": tp1_price, "quantity": tp1_qty, "reduceOnly": True, "workingType": "MARK_PRICE"},
                        {"symbol": symbol, "side": x_side, "type": "TAKE_PROFIT_MARKET", "stopPrice": tp1_price, "quantity": tp1_qty, "reduceOnly": True},
                        {"symbol": symbol, "side": x_side, "type": "TAKE_PROFIT", "stopPrice": tp1_price, "price": tp1_price, "quantity": tp1_qty, "reduceOnly": True, "timeInForce": "GTC", "workingType": "MARK_PRICE"},
                        {"symbol": symbol, "side": x_side, "type": "TAKE_PROFIT", "stopPrice": tp1_price, "price": tp1_price, "quantity": tp1_qty, "reduceOnly": True, "timeInForce": "GTC"},
                    ]
                    ...
                    if not tp1_created:
                        tp1_algo_variants = [
                            {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "TAKE_PROFIT_MARKET", "triggerPrice": tp1_price, "quantity": tp1_qty, "reduceOnly": "true", "workingType": "MARK_PRICE"},
                            {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "TAKE_PROFIT", "triggerPrice": tp1_price, "price": tp1_price, "quantity": tp1_qty, "reduceOnly": "true", "timeInForce": "GTC", "workingType": "MARK_PRICE"},
                        ]
        ...
        try:
            if tp2 > 0 and ((side=="LONG" and tp2>entry) or (side=="SHORT" and tp2<entry)):
                ...
                if tp2_qty > 0:
                    ...
                    tp2_variants = [
                        {"symbol": symbol, "side": x_side, "type": "LIMIT", "price": tp2_price, "quantity": tp2_qty, "timeInForce": "GTC", "reduceOnly": True},
                        {"symbol": symbol, "side": x_side, "type": "TAKE_PROFIT_MARKET", "stopPrice": tp2_price, "quantity": tp2_qty, "reduceOnly": True, "workingType": "MARK_PRICE"},
                    ]
                    ...
                    if not tp2_created:
                        tp2_algo_variants = [
                            {"symbol": symbol, "algoType": "CONDITIONAL", "side": x_side, "type": "TAKE_PROFIT_MARKET", "triggerPrice": tp2_price, "quantity": tp2_qty, "reduceOnly": "true", "workingType": "MARK_PRICE"},
                        ]
        ...
        # Invalidate all caches after a trade
        cache_key_pos = f"positions_{user_id or 'public'}"
        _positions_cache.pop(cache_key_pos, None)
        _positions_cache_time.pop(cache_key_pos, None)
        _live_positions_cache.pop(str(user_id or 'public'), None)
        _live_positions_cache_time.pop(str(user_id or 'public'), None)
        _open_orders_cache.pop(str(user_id or 'public'), None)
        _open_orders_cache_time.pop(str(user_id or 'public'), None)
        _conditional_cache.pop(user_id, None)
        cache_key_hist = f"trade_history_{user_id or 'public'}"
        _trade_history_cache.pop(cache_key_hist, None)
        _trade_history_cache_time.pop(cache_key_hist, None)
...
def partial_close_position(symbol, close_percent=None, close_qty=None, user_id=None):
    ...
    try:
        ...
        for ck in [f"positions_{user_id or 'public'}", f"trade_history_{user_id or 'public'}"]:
            _positions_cache.pop(ck, None)
            _trade_history_cache.pop(ck, None)
        _live_positions_cache.pop(str(user_id or 'public'), None)
        _open_orders_cache.pop(str(user_id or 'public'), None)
        _conditional_cache.pop(user_id, None)
...
def get_trade_history(user_id=None, force_refresh=False):
    ...
    # ✅ FIX: Increased TTL from 10s → 60s. Trade history doesn't need sub-minute refresh.
    ...
    if not _is_api_available() and not force_refresh:
            ...
            for pos in db_positions:
                ...
                if oid and oid not in existing_order_ids:
                    trade_history.append({
                        ...
                        'realized_pnl': float(pos.unrealized_pnl or 0),
                        'commission': 0.0,
                        'order_id': oid,
...
def get_user_trade_positions_with_tp_sl(user_id=None):
    ...
    try:
        ...
        positions = TradePosition.query.filter_by(user_id=user_id).order_by(
            TradePosition.status.desc(),
            TradePosition.updated_at.desc()
        ).limit(100).all()
...
def get_live_price(symbol, user_id=None):
    """
    Bulletproof price fetch with per-symbol caching.
    ...
    # ✅ FIX: Increased price cache TTL from 2s → 10s.
    # Live price comes from the frontend WebSocket directly; the server
    # only needs price for position-sizing calculations, not real-time display.
    ...
    if not _is_api_available():
        return _price_cache.get(cache_key, 0.0)
    ...
    if cache_key in _price_cache:
        ...
        print(f"⚠️ Using STALE cache [{stale_age:.1f}s old] for {symbol} = ${stale_price}")
        return stale_price

    print(f"❌ CRITICAL: NO PRICE AVAILABLE FOR {symbol} - returning 0")
    return 0.0
...
def get_live_balance(user_id):
    ...
    ✅ FIX: Added 30-second cache to prevent hitting futures_account on every page load.
    """
    global _balance_cache, _balance_cache_time
    now = time.time()
    cache_key = str(user_id)

    if cache_key in _balance_cache and (now - _balance_cache_time.get(cache_key, 0)) < BALANCE_CACHE_TTL:
        return (_balance_cache[cache_key], None)

    if not _is_api_available():
        return (_balance_cache.get(cache_key, (0.0, 0.0)), "API rate limited — using cached balance")
