...
def calculate_position_sizing(balance, entry, sl_type, sl_value):
    ...
    if sl_type == "SL Points":
        ...
        return {
            ...
            "error": None,
            "sl_mode": sl_type # Store sl_mode for API logic
        }
...
def place_binance_order(client, symbol, side, entry, sl_value, tp_list, sizing, user_units, user_lev):
    ...
    try:
        ...
        # 5. Calculate SL Price
        sl_price = entry
        # Note: The SL price sent to Binance does NOT include the +20 Pts or +0.2% buffer used for position sizing.
        # It uses the raw SL Value provided by the user (sl_value).
        if sizing.get("sl_mode") == "SL Points":
        ...
        # 5b. Place Stop Loss Order (A reverse order)
