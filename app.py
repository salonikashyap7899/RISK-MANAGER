...
def index():
    ...
    # FIX: Get Real Live Balance from Binance
    ...
    # Form inputs with default fallbacks to prevent "Undefined" errors
    ...
    # Calculate using your new formulas
    sizing = calculate_position_sizing(unutilized, entry, sl_type, sl_val)
    
    trade_status = None
    if request.method == "POST" and 'place_order' in request.form:
        if sl_val <= 0:
            trade_status = {"success": False, "message": "ERROR: SL is mandatory."}
        else:
            trade_status = execute_trade_action(unutilized, selected_symbol, side, entry, 
    ...
    # Ensure all variables used in index.html are defined here
