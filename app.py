...
def index():
    ...
    trade_status = session.pop('trade_status', None) # Survival via redirect
    selected_symbol = request.form.get("symbol", "BTCUSDT")
    order_type = request.form.get("order_type", "MARKET")
    entry = float(request.form.get("entry") or get_live_price(selected_symbol) or 0)
    ...
    if request.method == "POST" and 'place_order' in request.form:
        status = execute_trade_action(
            unutilized, selected_symbol, side, entry, order_type, sl_type, sl_val, 
            sizing, float(request.form.get("user_units") or 0), 
            float(request.form.get("user_lev") or 0), margin_mode, 
            float(request.form.get("tp1") or 0), int(request.form.get("tp1_pct") or 50), 
            float(request.form.get("tp2") or 0)
        )
