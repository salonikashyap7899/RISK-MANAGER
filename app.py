...
from logic import (
    ...
    # REMOVED: DAILY_MAX_TRADES, DAILY_MAX_PER_SYMBOL, TOTAL_CAPITAL_DEFAULT,
    # Constants now imported via 'config'
)
...
def index():
    ...
    stats = session["stats"]
    balance = session.get("capital", config.TOTAL_CAPITAL_DEFAULT) # Use config.TOTAL_CAPITAL_DEFAULT
    ...
    if request.method == "POST":
        ...
        sizing = calculate_position_sizing(
            unutilised_capital, # Use unutilised capital for risk
            entry,
            sl_type,
            sl_value
        )
        ...
        if 'place_order' in form:
            ...
            resp = execute_trade_action(
                balance=unutilised_capital, # Pass unutilised capital for risk check
                symbol=symbol,
                side=side,
                entry=entry,
                sl_type=sl_type, 
    ...
    return render_template(
        ...
        daily_max_per_symbol=config.DAILY_MAX_PER_SYMBOL,
    )
